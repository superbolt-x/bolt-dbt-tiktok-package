...
{#-
    ...
    Reads the day-grain incremental staging model (_stg_tiktok_campaigns_insights)
    directly, applies currency conversion, adds date parts, rolls up
    day/week/month/quarter/year, and joins campaign metadata built inline from the
    raw history table. Keeping the currency + rollup + metadata all in this one
    model means a selective run only materializes _stg_tiktok_campaigns_insights +
    this model (mirrors the Facebook day-grain + rollup pattern; the _stg model
    plays the day-grain feeder role).
-#}
...
{%- set stg_fields = adapter.get_columns_in_relation(ref('_stg_tiktok_campaigns_insights'))
                    |map(attribute="name")
                    |list
                    -%}
...
WITH
    ...
    (SELECT
        ...
        {%- endfor %}
    FROM {{ ref('_stg_tiktok_campaigns_insights') }}
    {%- if var('currency') != 'USD' %}
    ...
    WHERE date >= date_trunc('year', (select dateadd(day,-30,max(date)) from {{ ref('_stg_tiktok_campaigns_insights') }}))::date
    {%- endif %}
    ),
